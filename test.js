...
const CONFIG = {
    socket: {
        deviceIP: '192.168.0.45',  // EW11 브릿지 IP
        port: 8899
    }
};
...
// 엘리베이터 관련 패킷 (ESPHome cvnet.yaml 참고)
// F7 20 01 22 81 - Elevator Floors (층수 표시)
// F7 20 01 E1 81 - Elevator Current (실제 위치)
// F7 20 01 81 81 - Elevator Call 상태
const ELEVATOR_PACKETS = {
    floors: 'f720012281',      // 0x22: Floors
    current: 'f72001e181',     // 0xE1: Current position
    call: 'f720018181'         // 0x81: Call status
};
// 로그 출력 함수
const log = (...args) => {
...
function analyzePacket(data) {
    ...
    switch(deviceId) {
        ...
        case '207101110': //공조기 실행명령어
            ...
            break;
        case '2001e1': // Elevator Current (0xE1)
            analyzeElevatorPacket(data, 'current');
            break;
        case '200122': // Elevator Floors (0x22)
            analyzeElevatorPacket(data, 'floors');
            break;
        case '200181': // Elevator Call (0x81)
        case '208101':
...
// 엘리베이터 패킷 분석 함수
function analyzeElevatorPacket(data, type) {
    const packetDetails = Array.from(data).map(byte => `${byte.toString(16).padStart(2, '0')}`).join(' ');
    
    // 층수 추출 (ESPHome yaml 참고: offset 4 또는 11)
    let floor = null;
    if (type === 'floors') {
        // F7 20 01 22 81 [floor] ... (offset 5)
        floor = data[5];
        log('Elevator Floors:', floor, '| Packet:', packetDetails);
    } else if (type === 'current') {
        // F7 20 01 E1 81 ... [floor] (offset 11)
        floor = data[11];
        // ASCII 변환 가능성 확인 (0x31='1', 0x30='0')
        if (floor >= 0x30 && floor <= 0x39) {
            log('Elevator Current Floor:', String.fromCharCode(floor), '(ASCII)| Packet:', packetDetails);
        } else {
            log('Elevator Current Floor:', floor, '| Packet:', packetDetails);
        }
    }
}
// 난방 패킷 분석 함수
